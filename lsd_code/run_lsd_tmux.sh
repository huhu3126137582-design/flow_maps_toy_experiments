...
SESSION_NAME="${LSD_TMUX_SESSION:-flow-lsd}"
OUTPUT_DIR="$ROOT_DIR/outputs/lsd"
...
train_command+=" $python_q -u lsd_code/distill_lsd.py$extra_arguments"
train_command+=" --steps 2000 --resume auto --pilot-check;"
train_command+=" fi"
train_command+=" && grep -q '\"passed\": true' $pilot_summary_q"
train_command+=" && $python_q -u lsd_code/distill_lsd.py$extra_arguments --resume auto"
